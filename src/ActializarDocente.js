  ...
  const handleUpdate = () => {
    if (!Alumnoid) {
      ...
      var raw = JSON.stringify({
        id: Alumnoid,
        dnombre: Alumnonombre,
        dapellido: Alumnoapellido,
        correo: Alumnocorreo,
      });
      ...
      fetch(`http://localhost:3001/api/docente/${Alumnoid}`, requestOptions)
        .then((response) => response.json())
        .then((result) => {
    ...
    <div>
      ...
      {Datoalumnocard.length > 0 ? (
        <div>
          ...
          <p>Nombre: {Datoalumnocard[0].dnombre}</p>
          <p>Apellido: {Datoalumnocard[0].dapellido}</p>
